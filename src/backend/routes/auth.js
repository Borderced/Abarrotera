...
router.post('/auth/registraralumno', async (req, res) => {
  const { id_empleado, nombre, rol, usuario, contrasena} = req.body;
  ...
  try {
    const nuevoestudiante = await registrarAlumno(id_empleado, nombre, rol, usuario, contrasena);
...
router.post('/auth/registrardocente', async (req, res) => {
  const { id_producto, nombre_producto, precio_venta, precio_compra, stock, fecha_registro} = req.body;
  ...
  try {
    const nuevodocente = await registrarDocente(id_producto, nombre_producto, precio_venta, precio_compra, stock, fecha_registro);
